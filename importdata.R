...
# use factors?
Unique.Entrez = levels(factor(test.annots$Entrez.Gene))
#find rows that have a certain Entrez ID
test.annots$Entrez == '664862 /// 83924' # this works

df = list(39)

for (ID in 1: length(Unique.Entrez)){

 df[ID]= sum(test.annots$Entrez == Unique.Entrez[ID]) # this puts the number of rows that have that Entrez

}
setNames(df, Unique.Entrez)
